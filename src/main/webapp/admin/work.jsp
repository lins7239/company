    ...
    <div style="padding: 15px;">
        <fieldset class="layui-elem-field">
            ...
            <!--       <button class="layui-btn layui-btn-xs layui-btn-danger dw-batch-delete" dw-url="./delete.json">
              ...
              <hr>
              <table class="layui-table" id="worktable">
                  <colgroup>
                      <col width="150">
                      <col width="150">
                      <col width="200">
...
<script type="text/javascript">
			...
			$.ajax({
				url:"${APP_PATH}/team/selectAllWork.do",
				type:"post",
				success:function(result){
					build_works_table(result);
				}
				
			});
			
			function build_works_table(result){
	        	$("#worktable tbody").empty();
	        	var works=result.extend.works;
	        	$.each(works,function(index,work){
	        		var workTd = $("<td></td>").append(work.id);
					var worduriTd = $("<td></td>").append(work.worduri);
					var videouriTd = $("<td></td>").append(work.videouri);
					var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn").append("删除");
					delBtn.attr("del-id",work.id);
			...
			$(document).on("click",".delete_btn",function(){
				...
				if(confirm("确认删除吗?")){
					$.ajax({
						url:"${APP_PATH}/team/deleteWork.do",
						type:"post",
						data:{"id":id},
						success:function(result){
							if(result.code==200){
								layer.msg('<span style="font-size: 15px;">删除成功</span>',{
					   				 ...
					   				 time: 1000,
										 ...
										 window.location.href="${APP_PATH}/admin/work.jsp";
									 }
					   				}); 
							}
						}
