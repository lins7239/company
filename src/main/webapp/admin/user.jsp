...
<script type="text/javascript">
			...
			$.ajax({
				url:"${APP_PATH}/team/selectAllUser.do",
				type:"post",
			...
			function build_users_table(result){
	        	...
	        	$.each(users,function(index,user){
					...
					var userNoTd = $("<td></td>").append(user.userNo);
					var userNameId = $("<td></td>").append(user.userName);
					var passwordTd = $("<td></td>").append(user.password);
					var departmentTd = $("<td></td>").append(user.department);
					var classNameTd = $("<td></td>").append(user.className);	
			...
			$(document).on("click",".delete_btn",function(){
				...
				if(confirm("确认删除吗?")){
					$.ajax({
						url:"${APP_PATH}/team/deleteUser.do",
						type:"post",
						data:{"id":id},
						success:function(result){
							if(result.code==200){
								layer.msg('<span style="font-size: 15px;">删除成功</span>',{
					   				 ...
					   				 time: 1000,
										 ...
										 window.location.href="${APP_PATH}/admin/user.jsp";
									 }
					   				}); 
							}
						}
